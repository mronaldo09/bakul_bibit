...
class Order extends MY_Controller {
    ...
    public function update($id)
    {
        ...
        if ($this->order->where("id", $id)->update(["status" => $this->input->post("status")])) {
            $this->session->set_flashdata("success", "Databerhasil diperbarui.");
        } else{
            $this->session->set_flashdata("error", "Oops! Terjadi kesalahan!");
        }
        redirect(base_url("order/detail/$id"));
    ...
    public function pdf(){
        ...
        $this->dompdf->render();
        $this->dompdf->stream("laporan_orders.pdf", array("Attachment" =>0));
    }
    ...
    public function print(){
        ...
        $data["content"]        = $this->order->like("date", $keyword)->orderBy("date", "DESC")->get();
        $data['date']           = $this->input->post("keyword");
        $data["page"]           = "print_order";
