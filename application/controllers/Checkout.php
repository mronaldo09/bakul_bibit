...
class Checkout extends MY_Controller
{
	...
	public function create()
	{
		...
		if ($total >= 150000) {
			...
			if ($input->alamat_kirim == 1) {
				$ongkir = 20000;
			} elseif ($input->alamat_kirim == 2) {
				$ongkir	= 22000;
			} elseif ($input->alamat_kirim == 3) {
				$ongkir	= 23000;
			} elseif ($input->alamat_kirim == 4) {
				$ongkir	= 25000;
			} elseif ($input->alamat_kirim == 5) {
				$ongkir	= 26000;
			}
		};
		...
		if ($order = $this->checkout->create($data)) {
			...
			foreach ($cart as $row) {
				...
				$produk = $this->db->get_Where('product', array('id'=>$row['id_product']))->row_array();
				
				$simpan = array(
					'jumlah_stok' => $produk['jumlah_stok'] - $row['qty'],
				 );
				$this->db->where('id', $row['id_product']);
				$this->db->update('product', $simpan);
			}
