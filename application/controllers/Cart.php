...
class Cart extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data["content"]    = $this->cart->select([
            "cart.id", "cart.qty", "cart.subtotal", "product.title", "product.image", "product.price", "product.jumlah_stok"
        ])
            ->join("product")
            ->where("cart.id_user", $this->id)
            ->get();

        $data["page"]       = "pages/cart/index.php";
    ...
    public function add()
    {
        if (!$_POST || $this->input->post("qty") < 1) {
            ...
            redirect(base_url());
        } elseif($this->input->post("jumlah_stok") - $this->input->post("qty") < 0){
