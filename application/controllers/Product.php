...
class Product extends MY_Controller
{
    ...
    public function index($page = null)
    {
        ...
        $data["content"]    = $this->product->select(
            ["product.id", "product.jumlah_stok", "product.title AS product_title", "product.image", "product.price", "product.is_avaiable", "category.title AS category_title"]
        )
            ->join("category")
            ->paginate($page)
            ->get();
    ...
    public function search($page = null)
    {
        ...
        $data["title"]          = "Admin Produk";
        $data["content"]        =   $this->product->select(
            ["product.id", "product.title AS product_title", "product.image", "product.price", "product.is_avaiable", "category.title AS category_title"]
        )
            ->join("category")
    ...
    public function create()
    {
		...
		$this->product->table  = "category";

        if (!$this->product->validate()) {
            $data["title"]  = "Tambah Produk";
            $data["input"]  = $input;
            $data["form_action"]    = base_url("product/create");
            $data["category"]        = $this->product->where("parent_id !=", 0)->get();
            $data["page"]           = "pages/product/form";

            $this->view($data);
            return;
        }
		$this->product->table  = "product";
    ...
    public function print(){
        ...
        $data["content"]        = $this->product->orderBy("title", "DESC")->get();
        $data["page"]           = "pages/product/print";
